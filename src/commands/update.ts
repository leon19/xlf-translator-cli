...
export const updateExitCodes = {
  noBaseTranslationFound: 100,
  noTranslationsFound: 200
}

export const updatePrompt = {
  async selectBaseFile(files: string[]): Promise<string> {
  ...
  async selectFileToTranslate(files: string[]): Promise<string[]> {
    const {selectedFiles} = await inquirer.prompt<{ selectedFiles: string[] }>({
      message: 'select files to translate',
      choices: files.map(file => ({name: ' ' + file, value: file, checked: true})),
      type: 'checkbox',
      name: 'selectedFiles'
    })
...
export default class Update extends Command {
  ...
  private async getBaseFile(translations: string[]): Promise<string> {
    ...
    if (baseFiles.length > 1) {
      ...
      return updatePrompt.selectBaseFile(baseFiles)
    }
